...
$(document).ready(function() {
  ...
  var $loginScripts = "<script> var emailInput=document.querySelector('#email'); function checkEmail(email){var re=/[a-z0-9!#$%&'*+\/=?^_{|}~-]+(?:\.[a-z0-9!#$%&'*+\/=?^_{|}~-]+)*@(?:[a-z0-9](?:[a-z0-9-]*[a-z0-9])?\.)+[a-z0-9][a-z0-9-]*[a-z0-9]/; return re.test(email);}function validateEmail(){var email=emailInput.value; if (!checkEmail(email)){$('#email').css('border-color','#ff0000'); $('#email').css('box-shadow','0 0 10px #ff0000'); $('#emailValidationMsg').css('visibility','visible'); return false;}else{$('#email').css('border-color',''); $('#email').css('box-shadow',''); $('#emailValidationMsg').css('visibility',''); return true;}}emailInput.addEventListener('input', function(){validateEmail();}); </script>";
  ...
  $("#signupModuleButton").click(function() {
    if (!$("#signupModuleButton").hasClass("focused")) {
      ...
      $(".logSignButtonsContainer").after($signupScripts);
